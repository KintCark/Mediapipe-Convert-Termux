...
apt update && apt upgrade -y && apt-get install curl git gcc make build-essential python3 python3-dev python3-pip pipx python3-venv python-is-python3 -y && 0
pip install torch typing_extensions numpy Pillow requests pytorch_lightning absl-py && pipx install torch && pipx ensurepath && pip install safetensors && apt-get install google-perftools && apt install libgoogle-perftools-dev && pip3 install gradio spaces
